...
impl Handler {
    ...
    /// Drops the local connection state and requests that the platform disconnect the bearer.
    ///
    /// This is intended for application lifecycle transitions, where the platform may already
    /// have discarded the bearer without delivering its disconnect event to the plugin.
    pub async fn reset_connection(&self) {
        let device = self.connected_dev.lock().await.take();
        *self.notify_listeners.lock().await = vec![];
        ...
        if let Some(device) = device {
            if let Err(error) = device.disconnect().await {
                debug!("failed to disconnect while resetting BLE state: {error}");
            }
        }

        let _ = self.connected_tx.send(false);
        self.send_connection_update(false).await;
        on_disconnect.run().await;
    }
