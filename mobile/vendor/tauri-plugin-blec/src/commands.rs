...
// A complete Amaru telemetry snapshot can contain up to 45 BLE fragments.
// Keep a full snapshot in flight while the Tauri IPC channel drains it.
...
#[command]
pub(crate) async fn reset_connection<R: Runtime>(_app: AppHandle<R>) -> Result<()> {
    tracing::info!("Resetting BLE connection state");
    let handler = get_handler()?;
...
async fn subscribe_channel(
    ...
    let handler = get_handler()?;
    let (tx, rx) = tokio::sync::mpsc::channel(SUBSCRIPTION_BUFFER_CAPACITY);
    handler
        .subscribe(characteristic, service, move |data: Vec<u8>| {
            info!("subscribe_channel: {:?}", data);
            // A later complete snapshot supersedes an incomplete one. Do not
            // terminate the process when the receiver falls behind briefly.
