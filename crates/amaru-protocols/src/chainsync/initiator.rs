...
pub mod tests {
    ...
    use InitiatorState::*;
    use Message::*;
    use amaru_kernel::{Hash, RawBlock, Slot, hash::size::HEADER, make_header};
    ...
    impl ReadOnlyChainStore<BlockHeader> for MockChainStoreForIntersectPoints {
        ...
        fn load_header(&self, _hash: &Hash<HEADER>) -> Option<BlockHeader> {
        ...
        fn get_children(&self, _hash: &Hash<HEADER>) -> Vec<Hash<HEADER>> {
            todo!()
        }
        ...
        fn load_block(&self, _hash: &Hash<HEADER>) -> Result<RawBlock, StoreError> {
            todo!()
        }

        fn get_nonces(&self, _header: &Hash<HEADER>) -> Option<Nonces> {
            todo!()
        }

        fn has_header(&self, _hash: &Hash<HEADER>) -> bool {
            todo!()
        }
    }
}
