...
use crate::{
    EraHistory, Network, NetworkMagic, Slot, era_history,
    protocol_parameters::{
...
impl From<NetworkName> for &EraHistory {
    fn from(value: NetworkName) -> Self {
        match value {
            NetworkName::Mainnet => &era_history::MAINNET_ERA_HISTORY,
            NetworkName::Preprod => &era_history::PREPROD_ERA_HISTORY,
            NetworkName::Preview => &era_history::PREVIEW_ERA_HISTORY,
            NetworkName::Testnet(_) => &era_history::TESTNET_ERA_HISTORY,
        }
    }
}
