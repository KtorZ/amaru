...
use crate::{
    BlockHeight, Hash, Hasher, Header, HeaderBody, IsHeader, Point, Slot, Tip, cbor,
    hash::size::HEADER,
};
use std::{
    cmp::Ordering,
...
impl BlockHeader {
    ...
    pub fn new(header: Header, hash: Hash<HEADER>) -> Self {
        Self { header, hash }
    }
    ...
    pub fn parent_hash(&self) -> Option<Hash<HEADER>> {
...
impl IsHeader for BlockHeader {
    fn hash(&self) -> Hash<HEADER> {
        self.hash
    }

    fn parent(&self) -> Option<Hash<HEADER>> {
        self.header.header_body.prev_hash
    }
